...
public class asslisttutorAdapter extends ArrayAdapter<HashMap<String,Object>>{
    private LayoutInflater mInflater;
    private ArrayList<HashMap<String,Object>> mArray;
    private HashMap<String,Object> map;
    private int iView;
    private static Context context;
    public asslisttutorAdapter(Context c, int layoutView, ArrayList<HashMap<String, Object>> objects) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(convertView == null){
            ...
            holder.datetime = (TextView)convertView.findViewById(R.id.textView14);
...
//            holder.txtCount = (TextView)convertView.findViewById(R.id.txtBookCount);
        ...
        holder.name.setText("วิชา "+map.get(FieldConstants.name).toString()+" ห้อง "+map.get(FieldConstants.number).toString()+"\n วันที่ "+map.get(FieldConstants.Start_date).toString()+"-"+map.get(FieldConstants.End_date).toString());
        holder.datetime.setText("เวลา "+map.get(FieldConstants.Start_time).toString()+" "+map.get(FieldConstants.End_time).toString());
