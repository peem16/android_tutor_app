...
public class Rollcall extends AppCompatActivity implements AdapterView.OnItemClickListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (Build.VERSION.SDK_INT >= 11) {
            ...
            rollcall.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
        }
        else
        {
    ...
    private class Processrollcall extends AsyncTask<String, Integer, String> {
        ...
        protected String doInBackground(String... params) {
            ...
            Initial.httpConn = new ServerConnect(Rollcall.this);
            jResult = Initial.httpConn.getDataFromServer(keysv+PHPPath.Rollcall, ph);
...
//                        }
                    ...
                    if(!jObject.getString("desc").trim().equals("null")){
                        ...
                        for(int i=0; i<size2 ; i++){
                            ...
                            objMap.put(FieldConstants.date,jsonObject.getString(FieldConstants.datetime));
                            objMap.put(FieldConstants.id_tutor_compensate,jsonObject.getString(FieldConstants.id_tutor_compensate));
...
//                        }
                    ...
                    if(!jObject.getString("result2").trim().equals("null")){

                        JSONArray jArray3 = new JSONArray(jObject.getString("result2"));
                        int size3 = jArray3.length();
                        for(int i=0; i<size3 ; i++){
                            JSONObject jsonObject = jArray3.getJSONObject(i);
                            objMap = new HashMap<String, Object>();
                            objMap.put(FieldConstants.title, "3");
...
//                        }
                    ...
                    TextView empty  =(TextView) findViewById(R.id.emptyList3);
                    empty.setVisibility(VISIBLE);
                    empty.setText("คุณไม่มีตารางสอน");
...
//                AppStatus.setPausing(false);
                mAdapter = new CategoriesAdapter(getApplication(), R.layout.test, objArrayList);
